...
describe("PATCH /api/articles/:article_id", () => {
  ...
  test.only("400: responds with a bad request error if inc_votes property is missing from request", () => {
    return request(app)
      .patch("/api/articles/1")
      .send({  })
      .expect(400)
      .then(({ body }) => {
        expect(body.msg).toMatchObject("bad request");
      });
  })
  ...
  test("400: responds with a not found error if article_id is valid but does not exist", () => {
    return request(app)
      ...
      .expect(404)
      .then(({ body }) => {
        expect(body.msg).toMatchObject("not found");
      });
  })
});
